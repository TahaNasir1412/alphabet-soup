...
10. ENGLISH-ONLY META FIELDS (this is the rule models most often ignore for the LONGER fields — check yourself before returning): regardless of ${lang}, "niche", "intent", "intent_summary", "geo_reason", "niche_notes", "dialect_used", and the "question"/"options" text inside "suggested_clarifications" must ALWAYS be written in English, so the person running this tool (who may not read ${lang}) can understand what you detected. This applies EQUALLY to "niche_notes" even though it is a longer paragraph — do not switch back into ${lang} just because the field is long-form prose. Every other text field — everything inside "sweeps" — must be written in ${lang}, per rule 2. Do not mix these two groups within a single field.
...
For ANY language: if this specific language/keyword has a common source of spelling ambiguity real searchers actually type differently — Arabic hamza confusion (أ/إ/آ vs bare ا) or ة vs ه, French/Spanish accents included vs dropped, German ä/ö/ü/ß vs ae/oe/ue/ss — provide up to 5 alternate valid spellings of the exact base keyword in "sweeps.s12_spelling_variants". Two hard rules: (a) NEVER include the exact base keyword string itself as typed ("${keyword}") — it is already covered by every other sweep, so including it wastes every single query in its suffix crawl on guaranteed duplicates; (b) every entry must be a DISTINCT string — do not repeat the same variant twice. Return an empty array if no such ambiguity genuinely applies to this keyword.
...
app.post('/api/sort', async (req, res) => {
  ...
  // NOTE: this endpoint processes exactly ONE batch per call — no internal
  // chunking loop, no artificial delays. Chunking large keyword lists (and
  // pacing between chunks to respect rate limits) is the CLIENT's job now.
  // Doing it server-side used to mean a single request could run for
  // minutes on a large collected list, which always loses on a serverless
  // timeout no matter how high it's raised. Many small fast requests beat
  // one giant slow one on Vercel.
  const prompt = buildSortPrompt(keyword, niche, intent_summary, keywords);
  ...
  try {
    ...
    const clean = raw.replace(/```json|```/g, '').trim();
    const parsed = JSON.parse(clean);
    return res.json({ relevant: parsed.relevant || [], not_relevant: parsed.not_relevant || [], _source: usedProvider });
  } catch (e) {
    lastErr = e.message;
    console.error(`Sort batch (${provider}) failed: ${e.message}`);
  }
  ...
  if (provider !== 'groq' && PROVIDERS.groq.key) {
    try {
      ...
      const clean = raw.replace(/```json|```/g, '').trim();
      const parsed = JSON.parse(clean);
      return res.json({ relevant: parsed.relevant || [], not_relevant: parsed.not_relevant || [], _source: 'groq', _fallback_from: provider });
    } catch (e2) {
      lastErr = `${provider}: ${lastErr} | groq: ${e2.message}`;
      console.error(`Sort batch groq fallback also failed: ${e2.message}`);
    }
  }

  // Honest failure for this batch — nothing vanishes, just left unsorted (caller puts it in Review).
  res.json({ relevant: [], not_relevant: keywords, _source: 'none', _error: lastErr });
});
